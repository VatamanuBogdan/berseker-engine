...
glm::vec3 ConvexGeometry::FindFurthestFor(const glm::vec3 &direction, const glm::mat4 &model) const {
...
Collider::Collider(std::vector<ConvexGeometry> &&convexGeometries, const glm::mat4 &model)
	  : convexGeometries(std::move(convexGeometries)), model(model) {
}

const glm::mat4 &Collider::GetModel() const { return model; }

void Collider::SetModel(const glm::mat4 &model) { this->model = model; }
...
bool GJKCollisionStrategy::AreColliding(const Collider &collider1, const Collider &collider2) {
	...
	geometry1Model = &collider1.GetModel();
	geometry2Model = &collider2.GetModel();

	for (auto i = 0; i < geometry1.size(); i++) {
		for (auto j = 0; j < geometry2.size(); j++) {
			convexGeometry1 = &geometry1[i];
			convexGeometry2 = &geometry2[i];
...
glm::vec3 GJKCollisionStrategy::SupportVertexForDirection(const glm::vec3 &direction) {
	auto vert1 = convexGeometry1->FindFurthestFor(direction, *geometry1Model);
	auto vert2 = convexGeometry2->FindFurthestFor(-direction, *geometry2Model);
